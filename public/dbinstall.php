...
if ($request->isGet()) {
  ...
  // Check if templates compile dir is writable.
  if (is_writable(SMARTY_COMPILE_DIR)) {
    echo('Templates compile directory is writable.<br>');
  } else {
    echo('<font color="red">Error: Templates compile directory directory is not writable.</font><br>');
  }
  ...
  if (file_exists(APP_CONFIG . '/config.php')) {
    ...
    $file = fopen(APP_CONFIG . '/config.php', 'r');
    $line = fgets($file);
    if (strcmp("<?php\n", $line) !== 0 && strcmp("<?php\r\n", $line) !== 0) {
      echo('<font color="red">Error: config.php file does not start with a PHP opening tag.</font>  See <a href="https://www.anuko.com/lp/tt_33.htm" target="_blank">explanation</a>.<br>');
    }
    fclose($file);
  } else {
    echo('<font color="red">Error: config.php file does not exist.</font><br>');
  }
  ...
  if (version_compare(phpversion(), '8.3', '>=')) {
    echo('<font color="red">Error: This app was not fully tested with PHP version: '.phpversion().'.</font><br>');
  } else {
...
if ($_POST) {
  ...
  if (array_key_exists('convert12204to12205', $_POST)) {
    // Feature store sessions in database
    ttExecute("ALTER TABLE `tt_users` CHANGE `password` `password` VARCHAR(255)");
